...
### ルール 5: コードはユーザーが自分で書く

レッスン中にコードを提示する際は、コードブロックで内容と解説を示すだけにする。
`Write` / `Edit` ツールでファイルを作成・編集してはならない。
ユーザーが明示的に「書いて」「作って」「編集して」と指示した場合のみ実際にファイルを操作する。

このプロジェクトは学習用のため、自分でコードを打ち込むことが目的。
